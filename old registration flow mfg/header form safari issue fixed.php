 ...
 * Note: With the new JS, this is only used if JS fails
 * Keep this for progressive enhancement
 */
...
function mfg_medical_form_bridge() {
    ...
    $base_url = 'https://widget.myfloridagreen.com/PatientEnrollment/Registration/';
    $token_part = '4300570066004F00790036002F0068007800690051003D00';
    $token = $token_part . '-' . $token_part;

    $redirect_url = $base_url . $token .
                    '%C2%B6' . rawurlencode($phone) .
                    '%C2%B6' . rawurlencode($email_hex);
...
/**
 * Display error message if present (add to your page template)
 */
...
function mfg_show_error_message() {
    ...
    <script>
    (function() {
        // Show error message using browser alert (or customize with your modal)
